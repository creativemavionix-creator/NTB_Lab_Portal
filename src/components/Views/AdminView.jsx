...
export default function AdminView() {
  ...
  return (
    <div className="flex-1 p-4 md:p-6 space-y-6 pb-20 bg-[#edf3f9] text-slate-800 min-h-full font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4 min-w-0">
        <div className="min-w-0 flex-1">
          <h2 className="text-base md:text-lg font-bold text-[#1e3a8a] tracking-tight uppercase flex items-center gap-2 min-w-0">
            <Database className="text-rose-500 shrink-0" size={20} />
            <span className="truncate">NTB Admin Master Connection</span>
          </h2>
      ...
      {/* Admin Tabs */}
      <div className="flex border-b border-slate-200 gap-2 overflow-x-auto min-w-0 pb-1" role="tablist" aria-label="Admin master records tabs">
        {['Personnel', 'Structure', 'Testing'].map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-xs font-bold border-b-2 transition-colors shrink-0 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none ${
              activeTab === tab 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 min-w-0">
        {masterGroups[activeTab].map((group) => (
          <div key={group.name} className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs space-y-3 min-w-0">
            <div className="flex items-center justify-between border-b border-slate-200 pb-2 min-w-0 gap-2">
              <h3 className="font-bold text-slate-900 text-xs uppercase tracking-wider truncate min-w-0 flex-1">{group.name}</h3>
              <span className="text-[10px] text-indigo-700 font-bold bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100 shrink-0">
                {group.key ? 'Live State Editable' : 'System Fixed'}
              </span>
            </div>

            {/* List of items */}
            <ul className="space-y-1.5 text-xs text-slate-700 font-medium max-h-60 overflow-y-auto pr-1 min-w-0">
              {group.items.map((item, idx) => (
                <li key={idx} className="flex items-center justify-between p-2 rounded bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-colors min-w-0 gap-2">
                  <span className="truncate pr-2 min-w-0 flex-1">{item}</span>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[9px] text-emerald-700 font-bold flex items-center gap-1 shrink-0">
                    ...
                    {group.key && (
                      <button
                        ...
                        className="text-slate-400 hover:text-rose-600 p-0.5 rounded focus-visible:ring-1 focus-visible:ring-rose-500 focus-visible:outline-none transition-colors cursor-pointer shrink-0"
                        title={`Delete ${item}`}
            ...
            {group.key && (
              <form onSubmit={(e) => handleAddItem(group.key, e)} className="flex items-center gap-2 pt-2 border-t border-slate-100 min-w-0">
                <input
                  type="text"
                  ...
                  className="flex-1 min-w-0 text-xs p-2 bg-slate-50 border border-slate-200 rounded focus:bg-white focus:outline-none font-medium"
                />
                <button
                  type="submit"
                  className="px-3 py-2 bg-[#1e3a8a] hover:bg-[#1e293b] text-white font-bold rounded text-xs flex items-center gap-1 cursor-pointer transition-colors shrink-0 shadow-2xs focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none"
                >
                  <Plus size={14} /> Add
                </button>
              </form>
